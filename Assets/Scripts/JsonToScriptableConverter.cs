...
{
    private string jsonFilePaht = "";                                   //JSON 파일 경로 문자열 값
    ...
    private void OnGUI()
    {
        ...
        EditorGUILayout.LabelField("SelectedFile : ", jsonFilePaht);
        EditorGUILayout.Space();
    ...
    private void ConverJsonToScriptableObjects()                        //JSON 파일을 ScriptableObject 파일로 변환 시켜주는 함수
    {
        //폴더 생성
        ...
        //JSON 파일 읽기
        string jsonText = File.ReadAllText(jsonFilePaht);               //JSON 파일을 읽는다.
